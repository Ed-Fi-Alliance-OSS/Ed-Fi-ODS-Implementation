...
(
	...
	WHERE 
		views.name = 'vw_studentHomelessProgramAssociation'
		AND schemas.name = 'nmped_rpt'
)

DROP VIEW nmped_rpt.vw_studentHomelessProgramAssociation;
GO

CREATE VIEW nmped_rpt.vw_studentHomelessProgramAssociation AS 
...
FROM	edfi.StudentHomelessProgramAssociation WITH (NOLOCK)
			...
			INNER JOIN edfi.Descriptor Descriptor_1 WITH (NOLOCK)
