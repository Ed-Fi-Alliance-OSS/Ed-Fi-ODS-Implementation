 ...
 * Date:	10/24/2023
 * Desc:	This script creates a view for basic SPED Program Service information
 *			The view should return one row for each Program Service and Program Service Provider
 *			
 * This view uses the following tables:
 *
 *  edfi.Student - Do we need it?
 *	edfi.Descriptor
 *	edfi.StudentSpecialEducationProgramAssociation
 *	nmped.StudentSpecialEducationProgramAssociationExtension
...
CREATE OR ALTER VIEW [nmped_rpt].[vw_StudentSpEdProgramServices]
AS 
...
SELECT DISTINCT
	...
	,S.FirstName + ' ' + S.LastSurname					AS [StudentFullName]
...
FROM nmped.StudentSpecialEducationProgramAssociation SSEPA WITH (NOLOCK)
