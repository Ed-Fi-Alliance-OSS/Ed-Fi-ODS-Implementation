...
docker cp ./bootstrap-sandbox.sql ed-fi-db-admin:/tmp/bootstrap-sandbox.sql
docker exec -i ed-fi-db-admin sh -c "psql -U postgres -d EdFi_Admin -f /tmp/bootstrap-sandbox.sql"
docker exec -i ed-fi-db-admin sh -c "rm /tmp/bootstrap-sandbox.sql"
